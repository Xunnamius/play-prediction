<?php
	...
	class STR
	{
		 ...
		 *
		 * @return String formatted string data
		 * @return Array (your formatted string, your formatted string with trailing symbols unmodified) if $cutoff_non_alphanumeric = TRUE
		 */
		public static function limit_words($string, $word_count_limit=7, $char_length_limit=35, $cutoff_non_alphanumeric=FALSE, $separator=' ')
		{
			...
			// Added in update 1.41
			if($cutoff_non_alphanumeric)
			{
				$return = array($new_string);
				...
				do
				{
					...
					if(!in_array($endchar, $terminators) && !ctype_alnum($endchar))
					{
						$invalid = TRUE;
						$new_string = substr($new_string, 0, $endpos);
					}
				} while($invalid);
				
				array_unshift($return, $new_string);
				return $return;
			}
