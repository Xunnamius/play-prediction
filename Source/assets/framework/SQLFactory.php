	...
	final class SQL
	{
		...
		public static function load_driver($driver)
		{
			...
			if(is_string($driver) && file_exists($path) && include_once $path)
			{
				...
				if(isset($instance))
				{
					$response = $instance->throw_errors(DG_DEBUG_MODE);
					
					if(isset($response) && $response === true) return $instance;
					else DEH::except('The "'.$driver.'" driver was found, but contained no properly formatted "throw_errors()" method with which to preform construction!', E_USER_WARNING, 2);
				}
				
				else DEH::except('The "'.$driver.'" driver was found, but contained no properly formatted "get_instance()" method with which to preform construction!', E_USER_WARNING, 2);
			}
			
			else DEH::except('SQL Driver "'.$driver.'" was not found', E_USER_ERROR, 2);
			return NULL;
		}
	}
